...
const UserSigninView = () => {
  ...
  const handleSubmit = (values) => {
    ...
    UserAPI.signup(modifiedValues)
      .then((res) => {
        if (res.status == 201) {
          let uid = res.data.data.id;
          const state = {
            uid: uid,
            contact_number: modifiedValues.contact_number,
          };
          navigate("/verify", { state: state });
        }
        if (res.status == 400) {
          if (res.data.contact_number !== "") alert(res.data.contact_number);
          if (res.data.email !== "") alert(res.data.email);
