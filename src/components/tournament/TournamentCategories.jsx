  ...
  return (
    <>
      {loaded & (tournaments?.length > 0)
        ...
        : "No Tournaments Categories Available. Add some to get started with!"}
    </>
  );
};
...
const Page = () => {
  ...
  const [tournamentWrapper, wrapperLoaded] = useAxios({
    url: `tournament/wrapper/${id}/`,
    method: "get"
  })

  if (!categoriesLoaded && !wrapperLoaded) {
    return <Shimmer />;
  }
    ...
    <>
      ...
      <div className="overflow-y-scroll maxh hide-scrollbar px-5">
        <div className="mt-7 mb-2 flex items-center justify-between">
          ...
          {!tournamentWrapper?.is_over && tournamentWrapper?.user_is_host &&
            <button
              onClick={() => {
