...
import { CATEGORIES_GENERATION, FORM_CONDITIONS, FORM_OPTIONS } from "../../services/misc.services";
...
export default () => {
    ...
    const removeSubCategory = (id) => {
        setSubTournaments(prev => {
            return prev.slice(id, 1)
        })
    }
    ...
    const handleSubSubmit = (values, { setSubmitting, resetForm }) => {
        if (editingIndex !== null) {
            ...
            const generatedCategories = generateCategoriesCombinations(
                values.participationType, values.subCategories,
                values.gender, tournamentWrapper?.sport,
                values.singlesRegistrationFees, values.doublesRegistrationFees, values.teamRegistrationFees
            )
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await axiosAuthRequest(`tournament/wrapper/${id}/get_categories/`, {
                    method: 'GET'
                }, false);
            ...
            const tourney = await axiosAuthRequest(`tournament/wrapper/${id}/`, {
                method: 'GET'
            }, false);
    ...
    const getCategoryObject = (pc, sc, g, trf = null) => new Object({
        participationType: pc || null, subCategory: sc || null, gender: g,
        teamRegistrationFees: trf
    })

    // participation type, sub category, .., ..., team reg singles, team reg doubles
    const generateCategoriesCombinations = (pt, sc, gender, sport, trs, trd, trf) => {
        const categories = [];
            ...
            if (pt === 'both') {
                for (const _pt of CATEGORIES_GENERATION[sport].pt) {
                    for (const _sc of CATEGORIES_GENERATION[sport].sc) {
                        categories.push(getCategoryObject(_pt, _sc, "male", _pt === "singles" ? trs : trd))
                        categories.push(getCategoryObject(_pt, _sc, "female", _pt === "singles" ? trs : trd))
                    }
                }
            } else {
                ...
                <div className="p-6 pt-2">
                    ...
                    >
                        {({ isSubmitting, values, submitForm }) => (
                            <Form>
                                <div>
                                    {FORM_CONDITIONS[tournamentWrapper?.sport].participationType && (
                                        <div className="mt-5">
                                            <div className="bg-gray-100 flex p-3 rounded-lg items-center">
                                                ...
                                                >
                                                    ...
                                                    {FORM_CONDITIONS[tournamentWrapper?.sport].participationType.map((type, idx) => (
                                        ...
                                        <div className="mt-5">
                                            <div className="bg-gray-100 flex p-3 rounded-lg items-center">
                                                ...
                                                >
                                                    ...
                                                    {FORM_CONDITIONS[tournamentWrapper?.sport].subCategories.map((category, idx) => (
                    ...
                    <h1 className="flex justify-center font-bold" id="added-tournament-categories" ref={addedCategoriesView}>Added Tournament Categories</h1>

                    {subTournaments?.map((subTour, id) => (
                        <div key={id} className="border-2 rounded-md my-2 p-4">
                            <h1 className="text-lg font-bold text-center mb-4">
                                {subTour.participationType && `${subTour.participationType} - `} {subTour.subCategory && `${subTour.subCategory} - `} {subTour.gender}
                            </h1>
                            {FORM_CONDITIONS[tournamentWrapper?.sport].subCategories && (
                                <div className="mb-2">
