...
const ParticipantScreen = () => {
  ...
  const navigateToParticipant = (participant_id) => {
    const state = {
      ...team.participantsDetails[participant_id],
      tid: team_id,
      is_host: tournament.user_is_host,
    };
    navigate(
      `/tournament/${team.parent_tournament}/team/${team.id}/participant`,
      { state }
    );
  };

  const acceptTeamParticipation = () => {
    TeamAPI.changeTeamParticipationStatus(team_id, {
  ...
  useEffect(() => {
    if (isteamReady) {
      let isAnyParticipantNotVerified = team.participantsDetails.some(
        ...
        <>
          ...
          <div>
            ...
            <div className="p-6">
              ...
              {/* {console.log(team.participantsDetails)} */}
              {team.participantsDetails.map((participant, index) => (
                ...
                >
                  ...
                  <div className="flex items-center">
                    <div className="mr-3">
                      <h1 className="flex justify-end text-gray-500">
                        #{index + 1}
                      </h1>
                    ...
                    <ChevronRightOutlinedIcon
                      ...
                      onClick={() => navigateToParticipant(index)}
                    />
                  </div>
                </div>
              ))}
              ...
              {tournament.user_is_host ? (
                <div className="flex justify-between ">
                  ...
                  {!enable ? (
                    <button className="bg-orange-200 cursor-not-allowed border-orange-200 text-white  border-2 m-1 flex p-3 rounded-lg font-semibold w-full justify-center">
                      Accept
                    </button>
                  ) : (
                    <button
